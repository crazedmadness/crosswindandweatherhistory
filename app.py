...
def history_snapshot_times(hour_offset, timezone_name="America/Los_Angeles"):
    ...
    ts_utc = pd.Timestamp.now(tz="UTC").floor("h") - pd.Timedelta(hours=int(hour_offset))
    try:
...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .shell.phone .timebits { gap:6px; }
        .shell.phone .chip { width:56px; margin:0 3px; }
...
@st.cache_resource
def get_xwind_timeline_component():
    return ensure_xwind_timeline_component()
...
if "top_n" not in st.session_state:
    st.session_state.top_n = LIVE_TOP_N_DEFAULT
...
# Reduce mobile/iPad jumpiness after the custom iframe triggers a Streamlit rerun.
components.html(
    """
    <script>
    (function() {
        try {
            const keep = window.parent.sessionStorage.getItem("xwind_keep_slider");
            const y = window.parent.sessionStorage.getItem("xwind_scroll_y");
            if (keep === "1") {
                window.parent.sessionStorage.removeItem("xwind_keep_slider");
                window.parent.sessionStorage.removeItem("xwind_scroll_y");
                setTimeout(function() {
                    const anchor = window.parent.document.getElementById("xwind-history-anchor");
                    if (anchor) {
...
if refresh:
    # Back to the old on-demand model: clear Streamlit/API caches only.
    # No disk/server-side precompute layer or timed server refresh is used.
    get_metars.clear()
...
# In wide mode, create the main columns here so the map placeholder starts at
# the same vertical height as the top of the nice timeline/control box.
...
if layout_mode == "Wide":
    ...
    with main_left_col:
        history_enabled, hour_offset, use_global = render_history_slider_controls(
            _history_title_for_topbar,
            phone=False,
        )
    with main_right_col:
        # This placeholder is intentionally created before any data fetch/build.
        # When filled later, the map top aligns with the timeline box instead of
        # starting below the left-side controls and row list.
        map_slot = st.empty()
...
def history_cache_key(active_icaos, use_global, min_wind, min_len, history_top_n):
    ...
    # Include the current UTC hour so the cached 24h bundle cannot drift as
    # time passes. Without this, old snapshots can be shown under new slider
    # labels, making one airport appear stuck across many hours.
    base_hour_key = pd.Timestamp.now(tz="UTC").floor("h").strftime("%Y-%m-%dT%H:00Z")
